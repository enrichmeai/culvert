...
- **Cloud-neutral by design.** The core depends only on contracts. GCP is the first full implementation; AWS is now a real (Java) adapter family — `BlobStore` (S3), `SecretProvider` (Secrets Manager), `Source`/`Sink` (SQS), and append-only `JobControlRepository` implementations on **both DynamoDB and Athena** (all three clouds' job control passes one shared contract test), with `Warehouse` (Athena) and observability hooks (CloudWatch) in progress. Azure Blob remains a single-method adapter skeleton.
