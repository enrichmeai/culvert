...
| **Warehouse** | MATCHES | All six methods — `query`, `execute`, `load_from_uri`, `merge`, `copy`, `table_exists` — match Java (`loadFromUri`, `tableExists` in Java camelCase). Return types match (`long`/`int` ↔ `int`; both are integer row counts). `load_from_uri` takes a required `LoadOptions` (write disposition + optional target partition) on both sides; the `LoadOptions`/`WriteDisposition` types mirror the Java record and enum. |
