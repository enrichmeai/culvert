...
- `gcp-pipeline-libraries/`: Reusable Python libraries and dbt macros (4-library architecture + tester + umbrella).
  ...
  - `data-pipeline-orchestration`: Coordination (Scheduling and dependency management using Airflow). Depends on core.
  - `data-pipeline-transform`: Data Modeling (SQL macros for audit columns and PII masking using dbt). **dbt only.**
  - `data-pipeline-tester`: Testing tools (Mocks, fixtures, and base test classes).
  - `data-pipeline-framework`: Umbrella package that installs all libraries.
