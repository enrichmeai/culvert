...
> **Last Updated:** March 2026 | **Version:** 1.0.29
...
- **Orchestration:** Reusable Airflow operators and DAG factories in `data-pipeline-orchestration`.
- **Transformation Macros:** Shared dbt macros in `data-pipeline-transform`.
- **Testing Utilities:** Base classes and mocks in `data-pipeline-tester`.
...
### 4-Library Model (Published as `data-pipeline-framework`)

Libraries are consumed from PyPI (`pip install data-pipeline-framework>=1.0.29`) and are **not embedded** in this repository.
...
gcp-pipeline-core (Foundation — no Beam, no Airflow)
   ...
   ┌────┴────┐
   ↓         ↓
gcp-pipeline-beam         data-pipeline-orchestration
(Ingestion — Beam)        (Control — Airflow)
        ↓                          ↓
data-pipeline-transform    data-pipeline-tester
(dbt macros)              (Test utilities)
```

| Library | Purpose | Tests |
|---------|---------|-------|
| [`data-pipeline-framework`](https://pypi.org/project/data-pipeline-framework/) | Umbrella package — installs all libraries below | - |
...
| `data-pipeline-orchestration` | Airflow DAGs, sensors, operators, DAG factory | 58 |
| `data-pipeline-transform` | dbt macros for audit columns and PII masking | - |
| `data-pipeline-tester` | Mocks, fixtures, base test classes | 101 |
...
├── data-pipeline-orchestration/                            # Control
├── data-pipeline-transform/                                # dbt macros
└── data-pipeline-tester/                                   # Testing utilities
...
cd ../data-pipeline-orchestration && PYTHONPATH=src:../gcp-pipeline-core/src python -m pytest tests/unit/ -q
cd ../data-pipeline-tester && PYTHONPATH=src python -m pytest tests/unit/ -q
...
| data-pipeline-orchestration | 58 |
| data-pipeline-tester | 101 |
