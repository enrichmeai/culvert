...
def main(argv=None):
    parser = argparse.ArgumentParser(
        prog="data-pipeline-docs",
        description="Access the Culvert data pipeline framework's documentation and project assets bundled with the package.",
    )
    ...
    if args.command == "list":
        ...
        print(f"  {dest_path}/deployments/         — Full deployment source (configs, tests, generators)")
        print(f"  {dest_path}/test_data/           — Sample CSV and .ok trigger files")
        print(f"  {dest_path}/.gitignore, etc.     — Root config files")
        print()
        print("Next steps:")
        print("  1. cd", dest_path)
        print("  2. git init && git add -A && git commit -m 'Initial import from data-pipeline-framework'")
        print("  3. pip install gcp-pipeline-ref-ingestion        # Dataflow ingestion source")
        print("     pip install gcp-pipeline-ref-transform        # dbt ODP-to-FDP models")
        print("     pip install gcp-pipeline-ref-orchestration    # Airflow DAGs")
        print("     pip install gcp-pipeline-ref-cdp              # dbt FDP-to-CDP models")
        print("     pip install gcp-pipeline-ref-segment-transform  # Mainframe segment writer")
        print()
        print("Note: Shell scripts (scripts/) are git-only — clone the repo for those.")
