...
| Shared library | Same `data-pipeline-framework` — zero new library code required |
...
| **Concurrent systems** | 1 deployed (Generic); architecture supports N | Each system gets its own Terraform module, datasets (`odp_{system_id}`, `fdp_{system_id}`), and buckets; shared `data-pipeline-framework` library |
...
| **PyPI** | `data-pipeline-framework` library | Build-time | HTTPS (pip install) |
...
| `data-pipeline-framework` | 1.0.11 | Umbrella package (5 libraries) |
