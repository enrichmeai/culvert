...
-   **Generic (Excess Management)**: 3 sources → 2 FDP (JOIN pattern).
...
-   **Terraform**: `infrastructure/terraform/` (unified root module) + `infrastructure/terraform/systems/generic/` (system-specific resources).
-   **GitHub Actions**: 
...
-   **Privacy**: In-flight PII masking via `data-pipeline-transform` macros.
