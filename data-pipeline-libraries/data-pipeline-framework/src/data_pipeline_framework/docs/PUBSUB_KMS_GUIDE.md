...
The library provides an enhanced Pub/Sub sensor for event-driven pipeline triggers, enabling reliable, scalable, and secure file processing.
...
**Core component:** `BasePubSubPullSensor` in `gcp-pipeline-libraries/data-pipeline-orchestration/src/gcp_pipeline_core/orchestration/sensors/pubsub.py`
...
```
1. FILE LANDING
   Mainframe extract (daily batch) --> GCS Landing Bucket
   gs://landing-bucket/generic/customers/
     customers_1.csv    (data file)
     customers_2.csv    (data file)
     customers.csv.ok   (trigger file) <-- triggers notification

2. PUB/SUB NOTIFICATION
   OBJECT_FINALIZE event --> Pub/Sub Topic (generic-file-notifications, CMEK encrypted)
   Message: { "bucket": "landing-bucket", "name": "generic/customers/customers.csv.ok", "eventType": "OBJECT_FINALIZE" }

3. AIRFLOW SENSOR (PULL)
   Step 1: Pull message    -- Polls subscription every 30s (configurable, backpressure friendly)
   Step 2: Filter          -- Match .ok files only; ignore data files
   Step 3: Extract metadata -- Parse bucket, object path, system, entity, date
   Step 4: Push to XCom    -- Downstream tasks access via key 'file_metadata'
   Step 5: Acknowledge      -- Message removed from subscription; on failure, returns to queue for retry

4. DOWNSTREAM TASKS
   Discover Split Files --> Validate HDR/TRL --> Load to BigQuery ODP --> Transform via dbt
   (Uses XCom metadata to locate data files e.g. customers_1.csv, customers_2.csv)
```
