...
Or manually (substituting `{ENV}` with `int`, `staging`, or `prod`):
...
The shared libraries (`gcp-pipeline-core`, `gcp-pipeline-beam`, `data-pipeline-orchestration`, `data-pipeline-transform`, `data-pipeline-tester`) are published as Python packages to **PyPI** under the umbrella package `data-pipeline-framework`.
...
1. Build the Dataflow Flex Template container. The Dockerfile installs `data-pipeline-framework` from PyPI.
...
3. dbt macros are pulled from `data-pipeline-transform` via `dbt deps`.

### Step 2.3: Deploy Orchestration Unit

1. Upload Airflow DAGs to the Cloud Composer DAGs bucket (`gs://<composer-bucket>/dags/`).
2. Update Cloud Composer environment PyPI dependencies to include `data-pipeline-framework==1.0.11`.
