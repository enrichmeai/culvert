...
pip install data-pipeline-framework>=1.0.6
...
pip install -e gcp-pipeline-libraries/data-pipeline-orchestration/
pip install -e gcp-pipeline-libraries/data-pipeline-transform/
pip install -e gcp-pipeline-libraries/data-pipeline-tester/
```
...
cd gcp-pipeline-libraries/data-pipeline-orchestration && PYTHONPATH=src python -m pytest tests/unit/ -v
```
...
| 4 | dbt macro-paths | Set `macro-paths: ["macros", "shared_macros"]` in `dbt_project.yml`. Dockerfile copies macros from pip-installed `data-pipeline-transform`. Add `dbt_utils` to `packages.yml` |
...
| 2 | Composer PyPI packages | Add `data-pipeline-framework>=1.0.6` to Composer environment PyPI dependencies |
