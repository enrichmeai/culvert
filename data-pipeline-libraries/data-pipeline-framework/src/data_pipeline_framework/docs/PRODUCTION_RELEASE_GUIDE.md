...
The framework uses a **4-library model**. Libraries must be released and versioned before they can be consumed by production deployments. All libraries are published to **PyPI** and consumed via `pip install data-pipeline-framework`.
...
| `data-pipeline-orchestration` | Control Plane (Sensors, DAG Factory, Airflow operators) | PyPI |
| `data-pipeline-transform` | Transformation (shared dbt macros, PII masking) | PyPI |
| `data-pipeline-tester` | Testing (Mocks, fixtures, base test classes) | PyPI |

The umbrella package `data-pipeline-framework` installs all five libraries in the correct dependency order.

- **Current version:** `1.0.11`
- **PyPI:** https://pypi.org/project/data-pipeline-framework/
...
- **Production Steps**:
  ...
  2. Run `dbt deps` to pull the latest `data-pipeline-transform` macros from PyPI.
...
- **Production Steps**:
  ...
  2. Update Cloud Composer environment PyPI dependencies:
     ...
     gcloud composer environments update generic-prod-composer \
         ...
         --update-pypi-package data-pipeline-framework==1.1.0
     ```
...
2. **Shared Libraries**: Publish `data-pipeline-framework==<version>` to PyPI.
