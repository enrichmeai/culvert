"""
Application2 (Application B Application) - Apache Beam/Dataflow Pipeline
====================================================================

Purpose:
  Pipeline for loading Application2 mainframe extracts to BigQuery ODP tables.
...
Note: Unlike Application1 (which waits for 3 entities), Application2 immediately triggers
...
Usage:
  python application2_pipeline.py --entity=applications --input_pattern=gs://bucket/application2/applications/*.csv
"""
...
from application2_ingestion.pipeline.options import LOAPipelineOptions
...
# Application2-specific imports - ONLY configuration and schemas
from application2_ingestion.config import SYSTEM_ID
from application2_ingestion.schema import LOAApplicationsSchema, LOA_SCHEMAS
...
class LOAPipeline(BasePipeline):
    """
    Application2 (Application B Application) ODP Load Pipeline.
    ...
    def build(self, pipeline: beam.Pipeline):
        """Build the Application2 pipeline logic."""
        ...
        self.write_to_bigquery(
            ...
            dlq_path=f"gs://{self.config.get('gcp_project_id')}-dlq/application2/{self.run_id}/valid_failures"
        )
        ...
        self.write_to_bigquery(
            ...
            dlq_path=f"gs://{self.config.get('gcp_project_id')}-dlq/application2/{self.run_id}/validation_failures"
        )
...
def initialize_otel(run_id: str, entity_type: str, environment: str = "dev") -> bool:
    ...
    if exporter_type == "dynatrace" and dynatrace_url and dynatrace_token:
        config = OTELConfig.for_dynatrace(
            service_name="application2-pipeline",
        ...
        config = OTELConfig.for_console(service_name="application2-pipeline")
    else:
...
def run_application2_pipeline(argv=None, expected_count: Optional[int] = None):
    """
    Run the Application2 ODP load pipeline.
    Updated to use LOAPipeline class which inherits from BasePipeline.
    """
    options = GDWPipelineOptions(argv)
    application2_opts = options.view_as(LOAPipelineOptions)
    gdw_opts = options.view_as(GDWPipelineOptions)

    # entity = application2_opts.entity
    entity = getattr(application2_opts, 'entity', None) or 'applications'
    config_entry = LOA_ENTITY_CONFIG[entity]
    schema = config_entry["schema"]
    run_id = gdw_opts.run_id or generate_run_id(f"application2_{entity}")
    ...
    logger = configure_structured_logging(
        ...
        system_id=SYSTEM_ID,
        entity_type=entity,
        logger_name="application2_pipeline"
    )

    # Prepare configuration for BasePipeline
    pipeline_config = {
        'run_id': gdw_opts.run_id,
        'pipeline_name': f"application2_{entity}_load",
        'entity_type': entity,
    ...
    try:
        ...
        with OTELContext(run_id=run_id, system_id=SYSTEM_ID, entity_type=entity) as otel_ctx:
            ...
            # if expected_count is not None and not application2_opts.skip_reconciliation:
            if expected_count is not None and not gdw_opts.skip_reconciliation:
                with otel_ctx.span("reconciliation") as recon_span:
                    ...
                    recon_result = reconciler.reconcile_with_bigquery(
                        ...
                        destination_table=application2_opts.output_table,
                        error_table=application2_opts.error_table
                    )
