"""
Application1 (Application A) - Apache Beam/Dataflow Pipeline
=======================================================

Purpose:
  Pipeline for loading Application1 mainframe extracts to BigQuery ODP tables.
...
Usage:
  python application1_pipeline.py --entity=customers --input_pattern=gs://bucket/application1/customers/*.csv
"""
...
# Application1-specific imports - ONLY configuration and schemas
from application1_ingestion.config import SYSTEM_ID
from application1_ingestion.schema import EMCustomerSchema, EMAccountSchema, EMDecisionSchema, EM_SCHEMAS
...
class EMPipeline(BasePipeline):
    """
    Application1 (Application A) ODP Load Pipeline.
    ...
    def build(self, pipeline: beam.Pipeline):
        """Build the Application1 pipeline logic."""
        ...
        self.write_to_bigquery(
            ...
            dlq_path=f"gs://{self.config.get('gcp_project_id')}-dlq/application1/{self.run_id}/valid_failures"
        )
        ...
        self.write_to_bigquery(
            ...
            dlq_path=f"gs://{self.config.get('gcp_project_id')}-dlq/application1/{self.run_id}/validation_failures"
        )
...
def initialize_otel(run_id: str, entity_type: str, environment: str = "dev") -> bool:
    ...
    if exporter_type == "dynatrace" and dynatrace_url and dynatrace_token:
        ...
        config = OTELConfig.for_console(service_name="application1-pipeline")
    else:
...
class EMPipelineOptions(PipelineOptions):
    """Application1 Pipeline command-line options."""
...
def run_application1_pipeline(argv=None, expected_count: Optional[int] = None):
    """
    Run the Application1 ODP load pipeline.
    Updated to use EMPipeline class which inherits from BasePipeline.
    """
    options = GDWPipelineOptions(argv)
    application1_opts = options.view_as(EMPipelineOptions)
    gdw_opts = options.view_as(GDWPipelineOptions)

    entity = application1_opts.entity
    config_entry = EM_ENTITY_CONFIG[entity]
    schema = config_entry["schema"]
    run_id = gdw_opts.run_id or generate_run_id(f"application1_{entity}")
    ...
    logger = configure_structured_logging(
        ...
        system_id=SYSTEM_ID,
        entity_type=entity,
        logger_name="application1_pipeline"
    )

    # Prepare configuration for BasePipeline
    pipeline_config = {
        'run_id': gdw_opts.run_id,
        'pipeline_name': f"application1_{entity}_load",
        'entity_type': entity,
            ...
            if expected_count is not None and not gdw_opts.skip_reconciliation:
                with otel_ctx.span("reconciliation") as recon_span:
                    ...
                    recon_result = reconciler.reconcile_with_bigquery(
                        ...
                        destination_table=application1_opts.output_table,
                        error_table=application1_opts.error_table
                    )
