...
class TestBuildPubsubTriggerDag:
    def test_returns_dag(self, sample_config):
        from gcp_pipeline_orchestration.factories._dag_builders import build_pubsub_trigger_dag
...
class TestBuildIngestionDag:
    def test_returns_dag(self, sample_config):
        from gcp_pipeline_orchestration.factories._dag_builders import build_ingestion_dag
        with patch("airflow.models.Variable.get", side_effect=lambda k, **kw: kw.get("default_var", "")):
...
class TestBuildTransformationDag:
    def test_returns_dag(self, sample_config):
        from gcp_pipeline_orchestration.factories._dag_builders import build_transformation_dag
        with patch("airflow.models.Variable.get", side_effect=lambda k, **kw: kw.get("default_var", "")):
...
class TestBuildErrorHandlingDag:
    def test_returns_dag(self, sample_config):
        from gcp_pipeline_orchestration.factories._dag_builders import build_error_handling_dag
...
class TestBuildStatusDag:
    def test_returns_dag(self, sample_config):
        from gcp_pipeline_orchestration.factories._dag_builders import build_status_dag
...
class TestDagFactoryIntegration:
    ...
    def _make_factory(self, config, tmp_path):
        import yaml
        from gcp_pipeline_orchestration.factories.dag_factory_alias import DagFactory
    ...
    def test_stub_fallback_when_import_error(self, sample_config, tmp_path):
        ...
        with patch(
            "gcp_pipeline_orchestration.factories._dag_builders.build_status_dag",
