...
**Gaps resolved (Sprint 14):** `AuditEventPublisher` gap closed by `BigQueryAuditEventPublisher`
(S14 T14.6, issue #95). All 16 contract interfaces now have at least one real adapter — the v0.1.0 bar is met.
...
### Sprint 16 — Hardening + v0.1.0 release prep (epic #51) — NOT yet groomed to tickets
Per `06-sprint-plan-9-16.md`: security review (no-secret-logging, IAM, CVE),
operational runbook + SLO/alerting, version-bump to 0.1.0 + signed-artifact
...
1. ~~**`AuditEventPublisher`** — no adapter.~~ **RESOLVED 2026-06-02:** in scope for v0.1.0 → ticket #95 (S14 T14.6, BigQuery adapter).
