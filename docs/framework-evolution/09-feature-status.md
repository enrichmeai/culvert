...
| `AuditEventPublisher` | ✅ | **none** | — | 🔴 contract only |
...
1. **`AuditEventPublisher`** — contract exists, no adapter. Needed for v1.0.0? (e.g. a Pub/Sub or BigQuery audit publisher.) Currently nothing in the 9–16 plan adds one.
...
3. **`rows_processed` real value** — sentinel today; needs stages to expose row counts. Which sprint owns that? (Candidate: S14 DQ transform, which already counts rows.)
