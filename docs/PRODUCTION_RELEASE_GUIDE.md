...
The framework uses a **4-library model** (core, beam, orchestration, transform) plus supplementary packages (tester, framework). Libraries must be released and versioned before they can be consumed by production deployments. All libraries are published to **PyPI** and consumed via `pip install gcp-pipeline-framework`.
